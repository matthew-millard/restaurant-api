...
server.tool("get_menu", "Returns all available menu items with prices, grouped by category", async () => {
    const data = await apiRequest("/api/v1/menu");
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("place_order", "Place a new order. Requires customer name, email, pickup time, and at least one item.", {
...
server.tool("track_order", "Get the status and details of an existing order by its ID", {
    order_id: z.string().uuid().describe("The order UUID"),
}, async (args) => {
    const data = await apiRequest(`/api/v1/orders/${args.order_id}`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("cancel_order", "Cancel a pending order by its ID", {
